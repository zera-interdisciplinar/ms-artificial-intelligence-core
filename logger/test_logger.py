...
def test_info_prints_message_in_blue(capsys):
    ...
    assert out == f"{logger.BLUE}[INFO] booting up{logger.RESET}\n"
...
def test_error_prints_message_and_error_in_red(capsys):
    ...
    out = capsys.readouterr().out
    assert out == f"{logger.RED}[ERROR] failed to connect: timeout{logger.RESET}\n"
...
def test_debug_prints_message_in_cyan(capsys):
    ...
    out = capsys.readouterr().out
    assert out == f"{logger.CYAN}[DEBUG] state dump{logger.RESET}\n"
...
def test_warning_prints_message_in_yellow(capsys):
    ...
    out = capsys.readouterr().out
    assert out == f"{logger.YELLOW}[WARNING] retrying{logger.RESET}\n"
